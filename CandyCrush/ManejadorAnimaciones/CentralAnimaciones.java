...
import Animadores.AnimadorEstado;
...
public class CentralAnimaciones implements ManejadorAnimaciones{
	...
	public synchronized void animar_intercambio(Celda celda) {
		Animador animador = new AnimadorMovimiento(this, 2, 4, celda);
		agregar_animador_y_lanzar_pendientes(animador);
	}
	
	public synchronized void animar_cambio_foco(Celda celda) {
		Animador animador = new AnimadorCambioFoco(this, celda);
		agregar_animador_y_lanzar_pendientes(animador);
	}

	public synchronized void animar_detonacion(Celda celda) {
		Animador animador = new AnimadorDetonacion(this, celda, 60);
		agregar_animador_y_lanzar_pendientes(animador);
	}
	
	public synchronized void animar_caida(Celda celda) {
		Animador animador = new AnimadorCaida(this, 2, 3, celda);
		agregar_animador_y_lanzar_pendientes(animador);
	}
	
	public synchronized void animar_cambio_visibilidad(Celda celda) {
		Animador animador = new AnimadorVisibilidad(this, celda, 10);
		agregar_animador_y_lanzar_pendientes(animador);
	}

	public synchronized void animar_cambio_estado_tdp2(Celda celda, int color) {
		Animador animador = new AnimadorEstado(this, celda, color);
		agregar_animador_y_lanzar_pendientes(animador);
	}
	
	protected synchronized void agregar_animador_y_lanzar_pendientes(Animador animador) {
		ventana.notificarse_animacion_en_progreso();
		animadores_pendientes.add(animador);
		lanzar_pendientes();
	}
	
	protected synchronized void lanzar_pendientes() {
		Animador animador;
	...
	protected synchronized void agregar_animador_en_progreso(Animador animador) {
	...
	protected synchronized void incrementar_animadores_activos_y_actualizar_ultima_prioridad_lanzado(int prioridad_ultimo) {
	...
	public synchronized void notificarse_finalizacion_animador(Animador animador) {
	...
	protected synchronized void quitar_animador_en_progreso(Animador animador) {
		Celda celda_animador = animador.get_celda_asociada();
		List<Animador> animadores_celda = mapeo_celda_animadores.get(celda_animador);
		animadores_celda.remove(animador);
	}
	
	protected synchronized void decrementar_animadores_activos_y_actualizar_ultima_prioridad_lanzado() {
		cantidad_animadores_ultima_prioridad_lanzados --;
